...
def inv_exists(workspace, env_path):
    ...
    workspace.builder.create_inventory_file(cluster, env, {"envDefinition": {"content": {"envTemplate": {"name": "test", "artifact": "test-artifact"}}}})
...
def sd_data_provided(workspace):
    if not hasattr(workspace, 'extra_env'):
        workspace.extra_env = {}
    workspace.extra_env["SD_DATA"] = '{"test": "data"}'
...
def multiple_inv_exist(workspace, env1, env2):
    c1, e1 = env1.split('/')
    c2, e2 = env2.split('/')
    workspace.builder.create_inventory_file(c1, e1, {"envDefinition": {}})
    workspace.builder.create_inventory_file(c2, e2, {"envDefinition": {}})
