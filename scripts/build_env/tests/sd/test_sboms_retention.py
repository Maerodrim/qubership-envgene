"""
SBOM retention policy integration tests.

UC-SBOM-1..5, UC-SBOM-MIG, UC-SBOM-NEG — filesystem-based pruning policy.

Integration pattern: configuration is written as a real config.yml dict via
yaml.dump under CI_PROJECT_DIR, SBOM files are created as real on-disk files
with specific sizes/mtimes via TestHelpers.create_file, sboms_retention_policy()
reads and processes them end-to-end, and assertions verify real filesystem state.
...
class TestSbomRetention(BaseTest):
    ...
    def _prepare(self, tc_name: str, config: dict) -> Path:
        case_dir = self.feature_dir / tc_name
        if case_dir.exists():
            shutil.rmtree(case_dir)
        case_dir.mkdir(parents=True)
        _write_config(case_dir / "configuration" / "config.yml", config)
    ...
    def test_invalid_config_raises_validation_error(self):
        case_dir = self._prepare("UC-SBOM-NEGATIVE-2",
                                 {"sbom_retention": {"enabled": True, "keep_versions_per_app": "invalid"}})
...
class TestSbomMigration(BaseTest):
    ...
    def _prepare(self, tc_name: str) -> Path:
        ...
        if case_dir.exists():
            shutil.rmtree(case_dir)
        case_dir.mkdir(parents=True)
        _write_config(case_dir / "configuration" / "config.yml",
                      {"sbom_retention": {"enabled": True, "keep_versions_per_app": 10}})
        self.set_ci_project_dir(case_dir)
        return case_dir
...
class TestSbomRetentionBackwardCompat(BaseTest):
    ...
    def _prepare(self, case_name: str, config: dict) -> tuple[Path, Path]:
    ...
    def test_keep_versions_zero_raises_validation_error(self):
        case_dir, sboms_dir = self._prepare("keep-versions-zero",
                                            {"sbom_retention": {"enabled": True, "keep_versions_per_app": 0}})
        app_dir = sboms_dir / "app-a"
        app_dir.mkdir()
        for i in range(3):
            TestHelpers.create_file(app_dir / f"app-a-{i}.sbom.json", size=100)
...
class TestSbomRetentionEdgeCases(BaseTest):
    ...
    def test_app_subdir_with_only_subdirectories_not_deleted(self):
        case_dir, sboms_dir = self._prepare(
            ...
            {"sbom_retention": {"enabled": True, "keep_versions_per_app": 1}})
        app_dir = sboms_dir / "app-a"
        app_dir.mkdir()
