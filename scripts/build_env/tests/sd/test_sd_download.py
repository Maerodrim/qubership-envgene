...
FEATURE_TEST_DIR = "test_sd_download"
...
def _write_yaml(path: Path, content_dict: dict) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(yaml.dump(content_dict, default_flow_style=False))
...
class TestParseRegistry(BaseTest):
    ...
    def setup_method(self):
        self.feature_dir = self.output_dir / FEATURE_TEST_DIR / "parse_registry"
        self.feature_dir.mkdir(parents=True, exist_ok=True)

    def test_plain_dict_without_version_or_auth_config_returns_v1(self):
        data = {"name": "r", "mavenConfig": _MINIMAL_MAVEN}
        path = self.feature_dir / "regdef_v1_plain.yml"
        _write_yaml(path, data)
        result = parse_registry(_read_yaml(path))
    ...
    def test_version_field_set_to_other_value_returns_v1(self):
        ...
        path = self.feature_dir / "regdef_v1_version.yml"
        _write_yaml(path, data)
        result = parse_registry(_read_yaml(path))
        assert isinstance(result, Registry)

    def test_version_2_0_triggers_v2_path(self):
        data = {
            "name": "r",
            "version": "2.0",
            "authConfig": {"main": {"provider": "nexus", "authMethod": "anonymous"}},
            "mavenConfig": _MINIMAL_MAVEN_V2,
        }
        path = self.feature_dir / "regdef_v2_version.yml"
        _write_yaml(path, data)
        result = parse_registry(_read_yaml(path))
        assert isinstance(result, RegistryV2)

    def test_auth_config_key_triggers_v2_path_even_without_version(self):
        # authConfig key triggers v2 path; schema requires version="2.0" so it is included.
        data = {
            "name": "r",
            "version": "2.0",
            "authConfig": {"main": {"provider": "nexus", "authMethod": "anonymous"}},
        ...
        path = self.feature_dir / "regdef_v2_auth_config.yml"
        _write_yaml(path, data)
        result = parse_registry(_read_yaml(path))
        assert isinstance(result, RegistryV2)
...
class TestMavenConfigValidators(BaseTest):
    ...
    def setup_method(self):
        self.feature_dir = self.output_dir / FEATURE_TEST_DIR / "maven_config"
        self.feature_dir.mkdir(parents=True, exist_ok=True)
    ...
    def _make(self, domain: str, full_url: str = "") -> MavenConfig:
        ...
        path = self.feature_dir / "maven_config.yml"
        _write_yaml(path, {"mavenConfig": data})
        return MavenConfig.model_validate(_read_yaml(path)["mavenConfig"])
...
class TestMavenConfigValidatorsNegative(BaseTest):
    ...
    def setup_method(self):
        self.feature_dir = self.output_dir / FEATURE_TEST_DIR / "maven_config_negative"
        self.feature_dir.mkdir(parents=True, exist_ok=True)
    ...
    def test_missing_required_field_raises_validation_error(self):
        ...
        path = self.feature_dir / "maven_config_missing_domain.yml"
        _write_yaml(path, {"mavenConfig": data})
        with pytest.raises(ValidationError):
            MavenConfig.model_validate(_read_yaml(path)["mavenConfig"])

    def test_empty_repository_domain_name_still_becomes_slash(self):
        # Edge case: empty string → rstrip("/") + "/" = "/". No exception raised.
        data = {**_MINIMAL_MAVEN, "repositoryDomainName": ""}
        path = self.feature_dir / "maven_config_empty_domain.yml"
        _write_yaml(path, {"mavenConfig": data})
        m = MavenConfig.model_validate(_read_yaml(path)["mavenConfig"])
...
class TestMavenConfigV2Validators(BaseTest):
    ...
    def setup_method(self):
        self.feature_dir = self.output_dir / FEATURE_TEST_DIR / "maven_config_v2"
        self.feature_dir.mkdir(parents=True, exist_ok=True)

    def _make(self, domain: str) -> MavenConfigV2:
        data = dict(_MINIMAL_MAVEN_V2)
        data["repositoryDomainName"] = domain
        path = self.feature_dir / "maven_config_v2.yml"
        _write_yaml(path, {"mavenConfig": data})
        return MavenConfigV2.model_validate(_read_yaml(path)["mavenConfig"])
    ...
    def test_missing_auth_config_field_raises_validation_error(self):
        ...
        path = self.feature_dir / "maven_config_v2_no_auth.yml"
        _write_yaml(path, {"mavenConfig": data})
        with pytest.raises(ValidationError):
            MavenConfigV2.model_validate(_read_yaml(path)["mavenConfig"])
...
class TestRegistryV1ResolveAuth(BaseTest):
    ...
    def setup_method(self):
        self.feature_dir = self.output_dir / FEATURE_TEST_DIR / "registry_v1_auth"
        self.feature_dir.mkdir(parents=True, exist_ok=True)

    def _make_v1_registry(self, credentials_id: str = "") -> Registry:
        data = {"name": "my-nexus", "mavenConfig": _MINIMAL_MAVEN}
        if credentials_id:
            data["credentialsId"] = credentials_id
        path = self.feature_dir / f"regdef_v1_{credentials_id or 'anon'}.yml"
        _write_yaml(path, data)
        return Registry.model_validate(_read_yaml(path))

    def test_no_credentials_id_returns_none(self):
        reg = self._make_v1_registry(credentials_id="")
        assert reg.resolve_auth({"any": {"data": {"username": "u", "password": "p"}}}) is None

    def test_none_env_creds_returns_none(self):
        reg = self._make_v1_registry(credentials_id="nexus-creds")
...
class TestRegistryV2ResolveAuth(BaseTest):
    ...
    def _make_v2_registry(self, provider: str, auth_method: str, credentials_id: str = "",
        ...
        safe_name = f"regdef_v2_{provider}_{auth_method}_{credentials_id or 'anon'}_{name}.yml"
        path = self.feature_dir / safe_name
        _write_yaml(path, data)
        return RegistryV2.model_validate(_read_yaml(path))

    def test_anonymous_auth_method_returns_none(self):
        # UC-AD-SD-8 (anonymous): no Authorization header emitted.
        reg = self._make_v2_registry("nexus", AUTH_METHOD_ANONYMOUS)
        assert reg.resolve_auth({}) is None

    def test_anonymous_does_not_require_credentials_in_env(self):
        reg = self._make_v2_registry("nexus", AUTH_METHOD_ANONYMOUS, name="my-nexus-v2-b")
        assert reg.resolve_auth(None) is None

    def test_nexus_user_pass_returns_correct_basic_auth_header(self):
        # UC-AD-SD-3: nexus + user_pass → Base64-encoded Basic header.
        reg = self._make_v2_registry("nexus", AUTH_METHOD_USER_PASS, credentials_id="nexus-creds")
        result = reg.resolve_auth(_ENV_CREDS)
        assert result == _expected_basic("ci-bot", "s3cr3t")
        decoded = base64.b64decode(result["Authorization"].removeprefix("Basic ")).decode()
        assert decoded == "ci-bot:s3cr3t"

    def test_artifactory_user_pass_returns_correct_basic_auth_header(self):
        # UC-AD-SD-5: artifactory + user_pass → Base64-encoded Basic header.
        reg = self._make_v2_registry("artifactory", AUTH_METHOD_USER_PASS, credentials_id="nexus-creds")
        result = reg.resolve_auth(_ENV_CREDS)
        assert result == _expected_basic("ci-bot", "s3cr3t")
        decoded = base64.b64decode(result["Authorization"].removeprefix("Basic ")).decode()
        assert decoded == "ci-bot:s3cr3t"
    ...
    def test_auth_config_name_mismatch_raises(self):
        ...
        path = self.feature_dir / "regdef_v2_mismatch.yml"
        _write_yaml(path, data)
        reg = RegistryV2.model_validate(_read_yaml(path))
...
class TestSnapshotRejection(BaseTest):
    ...
    def setup_method(self):
        self.feature_dir = self.output_dir / FEATURE_TEST_DIR / "snapshot_rejection"
        self.feature_dir.mkdir(parents=True, exist_ok=True)
        self.appdef_dir = self.feature_dir / "AppDefs"
        self.regdef_dir = self.feature_dir / "RegDefs"

    def _no_op_plugins(self):
        from envgenehelper.plugin_engine import PluginEngine
        return PluginEngine(plugins_dir=str(self.feature_dir / "_no_plugins"))

    def test_snapshot_suffix_raises_value_error(self):
        with pytest.raises(ValueError, match="SNAPSHOT"):
            download_sd_by_appver("my-app", "1.2.3-SNAPSHOT", self._no_op_plugins())

    def test_snapshot_anywhere_in_version_raises(self):
        # Even unconventional strings containing SNAPSHOT are rejected.
        with pytest.raises(ValueError, match="SNAPSHOT"):
            download_sd_by_appver("my-app", "SNAPSHOT-1.2.3", self._no_op_plugins())

    def test_snapshot_substring_raises(self):
        with pytest.raises(ValueError, match="SNAPSHOT"):
            download_sd_by_appver("my-app", "1.0.0-SNAPSHOT-RELEASE", self._no_op_plugins())
...
class TestMultiplySdsExtended(BaseTest):
    ...
    def setup_method(self):
        self.feature_dir = self.output_dir / FEATURE_TEST_DIR / "multiply_sds"
        self.feature_dir.mkdir(parents=True, exist_ok=True)

    def _write_sds_json(self, sds: list, filename: str) -> list:
        path = self.feature_dir / filename
        path.write_text(json.dumps(sds))
        return json.loads(path.read_text())

    def test_single_sd_in_extended_mode_accepted(self):
        sds = self._write_sds_json([{"applications": []}], "single_sd.json")
        result = multiply_sds_to_single(sds, MergeType.EXTENDED)
        assert result == {"applications": []}

    def test_multiple_sds_in_extended_mode_raises(self):
        sds = self._write_sds_json([{"applications": []}, {"applications": []}], "two_sds.json")
    ...
    def test_multiple_sds_in_basic_mode_are_merged(self):
        ...
        sds = self._write_sds_json([{"applications": []}, {"applications": []}], "two_sds_basic.json")
        result = multiply_sds_to_single(sds, MergeType.BASIC)
        assert "applications" in result

    def test_empty_list_in_extended_mode_raises(self):
        # Empty list in EXTENDED mode — no SD to unpack → IndexError/ValueError.
        sds = self._write_sds_json([], "empty_sds.json")
        with pytest.raises((IndexError, ValueError)):
            multiply_sds_to_single(sds, MergeType.EXTENDED)

    def test_unsupported_provider_in_v2_raises(self):
        # Unsupported (provider, authMethod) combination → ValueError from _PROVIDER_HANDLERS.
        # aws + user_pass is not in _PROVIDER_HANDLERS.
        data = {
            "name": "my-aws-reg",
            "version": "2.0",
            "authConfig": {"main": {"provider": "aws", "authMethod": "user_pass", "credentialsId": "nexus-creds"}},
            "mavenConfig": _MINIMAL_MAVEN_V2,
        }
        path = self.feature_dir / "regdef_v2_aws_user_pass.yml"
        _write_yaml(path, data)
        reg = RegistryV2.model_validate(_read_yaml(path))
        with pytest.raises(ValueError, match="Unsupported auth configuration"):
            reg.resolve_auth(_ENV_CREDS)
