...
def mock_nexus(tmp_path_factory):
    ...
    for ver in ["v1", "v2.0", "v2.1", "v2.2"]:
        art_dir = base_dir / "release" / "org" / "test" / "test-artifact" / ver
        art_dir.mkdir(parents=True, exist_ok=True)
        with open(art_dir / f"test-artifact-{ver}.json", "w") as f:
            json.dump({"configurations": [{"maven_repository": "http://localhost:8000/release", "artifacts": [{"id": f"org.test:test-artifact:{ver}"}]}]}, f)
        with zipfile.ZipFile(art_dir / f"test-artifact-{ver}.zip", "w") as z:
